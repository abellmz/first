...
@section('content')
    <div class="container-fluid">
        ...
        <form action="{{route('wechat.response_text.store')}}" method="post">
            @csrf
{{--WechatService、ResponseTextController两类有关 引入rule模板--}}
{{--传出的参数   该参数：调用了ruleView方法  该方法返回 加载rule页面  最终赋值给该传出参数--}}
            {!! $ruleView !!}
                ...
                methods:{
                    emotion(){
                        ...
                        $('#keyword textarea').each(function () {
                            //表情  这个花括号好像是vue特有的语法，查查
                            hdjs.emotion({
                                //点击的元素，可以为任何元素触发 this指的当前textarea
                                btn: $(this).prev('a'),
                                //选中图标后填入的文本框
                                input: $(this),
                         //选择图标后执行的回调函数   表情
